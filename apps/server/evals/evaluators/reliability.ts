import { EvaluationOutput } from "../lib/types.js";
...
export const reliabilitySummaryEvaluator = {
  ...
  scorer: (data: any) => {
    ...
    const rows: CollectedRow[] = inputs.map((inp: any, i: number) => ({
      inputs: inp,
      outputs: outputs[i],
    }));
...
export const p95LatencySummaryEvaluator = {
  ...
  scorer: (data: any) => {
    const { outputs } = data;
    const latencies = outputs.map((out: EvaluationOutput) => out.latencyMs);
