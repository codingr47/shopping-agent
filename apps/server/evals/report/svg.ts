...
export function svgHorizontalBarChart(rows: BarChartRow[], options: { height?: number; width?: number; showValues?: boolean } = {}): string {
  ...
  for (const row of rows) {
    ...
    svg += `<g>`;
    svg += `<text x="10" y="${y + height / 2}" font-size="12" text-anchor="start" dominant-baseline="middle" font-weight="500">${escapeHtml(row.label.substring(0, 40))}</text>`;
...
export function svgGroupedBarChart(
  ...
  for (const row of rows) {
    ...
    // Row label
    svg += `<text x="10" y="${y + (height * series.length) / 2}" font-size="12" font-weight="500" text-anchor="start" dominant-baseline="middle">${escapeHtml(row.label.substring(0, 40))}</text>`;
